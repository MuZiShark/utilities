			...
			for(var typeid in smilies_array) {
				for(var page in smilies_array[typeid]) {
					for(var i in smilies_array[typeid][page]) {
						...
						str = str.replace(re, '<img src="' + STATICURL + 'editor/smiley/' + smilies_type[typeid][1] + '/' + smilies_array[typeid][page][i][2] + '" border="0" smilieid="' + smilies_array[typeid][page][i][0] + '" alt="' + smilies_array[typeid][page][i][1] + '" />');
					}
				}
			}
		}
	...
	if(!fetchCheckbox('bbcodeoff')) {
		if(allowimgcode) {
			...
			str = str.replace(/\[attachimg\](\d+)\[\/attachimg\]/ig, function ($1, $2) {
				...
				return '<img src="' + $('image_' + $2).src + '" border="0" aid="attachimg_' + $2 + (width!=null?'" width="' + width:'') + '" alt="" style="max-width:400px" />';
			});
