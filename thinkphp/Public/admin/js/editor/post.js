...
function validate(theform) {
	...
	if(trim(theform.subject.value) == "") {
		MMQ.showDialog('提示信息','请完成帖子标题栏','error',function(){
			theform.subject.focus();
		});
		
		return false;
	} else if(mb_strlen(theform.subject.value) > 90) {
		MMQ.showDialog('提示信息','您的标题超过 90 个字符的限制','error',function(){
			theform.subject.focus();
		});
		return false;
	}
	if(trim(message) == ""){
		MMQ.showDialog('提示信息','请完成帖子内容','error');
		return false;
	}
	...
	if((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars)) {
                                   MMQ.showDialog('提示信息','您的帖子长度不符合要求。');
//		MMQ.showDialog('提示信息','您的帖子长度不符合要求。\n\n当前长度: ' + mb_strlen(message) + ' 字节\n系统限制: ' + postminchars + ' 到 ' + postmaxchars + ' 字节','error');
		return false;
	}
	if(theform.fid.value=="-1"){
		MMQ.showDialog('提示信息','请选择要发贴的圈子','error',function(){
	...
	if(UPLOADSTATUS == 0) {
		...
		MMQ.showDialog('提示信息','您有正在上传的附件，请稍候，上传完成后帖子将会自动发表...', 'notice');
		AUTOPOST = 1;
		return false;
	}
	hideMenu();
...
function uploadAttach(curId, statusid, prefix) {
	...
	if(curId > 0) {
		...
		if(nextId == curId || in_array(statusid, [6, 8])) {
			...
			if(UPLOADFAILED > 0) {
				MMQ.showDialog('提示信息','附件上传完成！成功 ' + UPLOADCOMPLETE + ' 个，失败 ' + UPLOADFAILED + ' 个:' + FAILEDATTACHS);
				FAILEDATTACHS = '';
			}
			UPLOADSTATUS = 2;
			for(var i = 0; i < AID - 1; i++) {
...
function insertAttach(prefix, id) {
	...
	if(extensions != '' && (re.exec(extensions) == null || ext == '')) {
		...
		MMQ.showDialog('提示信息','对不起，不支持上传此类扩展名的附件。');
		return;
	}
	if(prefix == 'img' && imgexts.indexOf(ext) == -1) {
		reAddAttach(prefix, id);
		MMQ.showDialog('提示信息','请选择图片文件(' + imgexts + ')');
		return;
	}
...
function updateImageList(action) {
	...
	jQuery.get(MMQ.setting.URL + '/attachment/imagelist/pid/0/tid/0/imagetype/'+type+'/posttime/' + $('posttime').value,function(data){
