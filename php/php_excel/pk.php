...
for($col=$startColumnIndex; $col<$endColumnIndex; $col++) {
    $title = $workSheet->getCellByColumnAndRow($col, 3)->getValue();
    $content = $workSheet->getCellByColumnAndRow($col, 4)->getValue();
    $type = 1;
    $week++;
    $data .= "('$title','$content','$type','$week'),";
}
...
//生成SQL语句
$sql = "insert into ".$table." (title,content,type,week) values ".$data;
//执行操作
