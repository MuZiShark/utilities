...
function display($startDate, $endDate) {
    $str  = '';
    $str .= '宝宝'.get_baby_age_by_birthday($startDate, $endDate).'&nbsp;';
    $str .= '<font color="red">'.get_baby_period_by_birthday($startDate, $endDate).'</font>';
    $str .= '&nbsp;'.get_baby_days_by_birthday($startDate, $endDate).'天';
    $str .= '&nbsp;'.get_baby_week_by_birthday($startDate, $endDate).'周';
    $str .= '&nbsp;'.get_baby_month_by_birthday($startDate, $endDate).'月';
    echo $str;
}
...
function get_baby_age_by_birthday($startDate, $endDate) {
    ...
    if($age) {
        if(isset($age['y']) && $age['y'] >= 1) {
            $res = '已满一周岁';
        } else {
            if(isset($age['m']) && $age['m'] >= 1) {
...
function get_baby_month_by_birthday($startDate, $endDate) {
	$age = format($startDate, $endDate);
    if($age) {
        $year = isset($age['y']) ? $age['y'] : 0;
        $month = isset($age['m']) ? $age['m'] : 0;
        if($year >= 1) {
